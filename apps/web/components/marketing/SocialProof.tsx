    ...
    <section className="py-16 border-t border-gray-100 bg-gray-50/30">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-10">
          <p className="text-sm font-semibold text-gray-600 uppercase tracking-wider text-center sm:text-left">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-600">from early users</span>
          </div>
        </div>

        {/* Logos — grayscale wordmarks */}
        <ul className="flex flex-wrap items-center justify-center gap-x-8 sm:gap-x-14 gap-y-6" aria-label="Companies using HireBase">
          {LOGOS.map((name) => (
            <li
              key={name}
              className="text-xl font-bold text-gray-500 tracking-tight select-none transition-colors duration-300 hover:text-gray-700"
            >
              {name}
            </li>
          ))}

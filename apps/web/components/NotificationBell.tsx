/*
 * NotificationBell — candidate in-app notifications (ADR-0009, Phase 4).
 * Bell + unread badge + dropdown. Polls /api/notifications; CSRF is handled by
 * the web middleware (same-origin), so no token is needed.
 */
...
export default function NotificationBell() {
  ...
  const markAllRead = useCallback(async () => {
    ...
    try { await fetch("/api/notifications", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: "{}" }); } catch { /* optimistic */ }
  }, []);
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        aria-haspopup="true"
        aria-expanded={open}
        className="relative flex h-9 w-9 items-center justify-center rounded-lg text-gray-600 hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
      >
        ...
        {unread > 0 && (
          <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-semibold text-white" aria-hidden="true">
            {unread > 9 ? "9+" : unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <div role="menu" aria-label="Notifications" className="absolute right-0 z-50 mt-2 w-80 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg">
          ...
          <div className="max-h-96 overflow-y-auto">
            {items.length === 0 ? (
              <p className="px-4 py-6 text-center text-sm text-gray-500">Nothing new yet.</p>
            ) : (

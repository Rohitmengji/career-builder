/*
 * @career-builder/ui — shared UI primitives for ALL apps (web app-chrome:
 * marketing, jobs, auth, job-detail, system; and admin: dashboard, editor,
 * settings, etc.). One source of truth for buttons, inputs, cards, layout,
 * badges, alerts, empty states — so every page across every app is consistent,
 * accessible (WCAG-AA), and responsive. See docs/DESIGN_SYSTEM.md.
 *
 * Each app re-exports this from its local components/ui so existing
 * `@/components/ui` imports keep working unchanged.
 *
 * Tenant-themed renderer blocks (renderer) keep their own theme tokens; these
 * primitives use the fixed product brand (blue-600).
 *
 * NOTE: classes here live outside each app's source tree, so every consuming
 * app must add `@source` for this package in its globals.css (Tailwind v4
